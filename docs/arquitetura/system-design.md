...
- Data da versao: 2026-08-18 (**v0.6**)
- Status: **Dominio aprovado com ressalva; `lib/json` e `lib/output` entregues.** Etapa 2 em andamento, sem decisao pendente que a bloqueie
...
| `lib/json` | Extrair campos de resposta de forma resistente a variacao de formatacao e a valores com delimitadores, **dentro de um teto de entrada de 256 KiB** | Corpo de resposta ate 256 KiB | Valores de campo | Nenhuma dependencia externa | ✅ **Entregue.** DP-08 fixou **interpretador proprio em shell, sem `jq`**. Atende RNF-11, requisito de maior criticidade do componente. **Teto de entrada de 256 KiB fixado na v0.6 por medicao** — reduzido de 4 MiB, que extrapolava para ~86 s de analise. O teto e uma decisao de projeto consciente, nao uma limitacao acidental: define uma fronteira explicita em vez de deixar a degradacao aparecer como travamento em producao |
