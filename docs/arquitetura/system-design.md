...
- Data da versao: 2026-08-18 (**v0.4**)
- Status: **Camada de dominio implementada e aprovada com ressalva pelo QA.** Documento corrigido a partir das divergencias devolvidas pelo ciclo
...
### ⚠️ Invariante arquitetural desta versao — nao reabrir
...
|---|---|
| Ausencia de interpretador JSON nativo no shell (RES-08) | Se resolvido por expressao regular, contamina toda a camada de adaptadores com fragilidade silenciosa. Exige decisao em DP-08 antes do desenho final de `lib/json` |
...
| Divergencia de utilitarios entre plataformas (GNU versus BSD) | Empurra chamadas de sistema para uma camada de compatibilidade; sem isso, o codigo de comando fica poluido de condicionais de plataforma |
...
| `lib/preflight` | Verificar versao minima de shell e presenca dos utilitarios obrigatorios | Ambiente de execucao | Sucesso ou falha nomeando o item ausente | Utilitarios do sistema | Atende RNF-01 e RNF-02 |
...
| `lib/config` | Localizar, validar e carregar perfil de credencial; aplicar e verificar permissoes restritas; carregar a raiz remota permitida | Caminho de configuracao, seletor de perfil | Credenciais em memoria, raiz permitida | Sistema de arquivos | Atende RF-03, RF-05, RNF-04, RNF-20. Formato final depende de DP-05 e DP-11 |
...
| `lib/http` | Unico ponto de saida de rede. Montagem de requisicao, injecao de credencial fora do `argv`, retentativa com recuo, captura de codigo HTTP e de `X-Dropbox-Request-Id` | Endpoint, cabecalhos, corpo, arquivo | Corpo de resposta, codigo HTTP, metadados de diagnostico | `cURL`, `lib/errors`, `lib/log` | Concentra RNF-03, RNF-06, RNF-07 e RNF-08 |
...
| `lib/json` | Extrair campos de resposta de forma resistente a variacao de formatacao e a valores com delimitadores | Corpo de resposta | Valores de campo | Estrategia definida em DP-08 | Atende RNF-11. **Desenho bloqueado por DP-08** |
...
| `lib/walk` | Percorrer arvore local ou remota aplicando padroes de inclusao e exclusao | Caminho raiz, padroes | Sequencia de itens | `lib/path` | Suporta RF-12 e RF-13 |
...
| `lib/output` | Manter o modelo de resultado unico e renderiza-lo em **duas apresentacoes**: legivel por humano e estruturada por maquina. Selecionar a apresentacao por deteccao de terminal associado, sobreponivel por sinalizador. Separar saida padrao de saida de erro | Resultado estruturado do comando | Texto renderizado | `lib/cli` para o contexto de execucao | Atende RF-28 e RNF-19. **Nenhum comando emite texto diretamente** — regra verificavel por analise estatica |
...
- **Ambientes necessarios:** estacao de desenvolvimento com shell alvo, `cURL` e utilitarios POSIX; container de referencia reproduzindo a distribuicao minima suportada; aplicativo Dropbox dedicado a testes, separado do de producao, preferencialmente com acesso restrito a pasta do aplicativo.
- **Dependencias locais:** `cURL`; analisador estatico de shell (`shellcheck`); arcabouco de teste de shell (`bats` ou equivalente, a definir pelo Senior Developer); interpretador JSON conforme DP-08; formatador de shell opcional.
...
| Redacao de erro | ~n^1.5 — 4 KiB em 0,02 s; 256 KiB em 4,56 s | **Unico ponto de atencao remanescente.** Aceitavel para mensagens de erro reais, que sao pequenas; degrada em corpo de erro anomalo. Vigiar se `lib/http` passar a submeter respostas grandes a redacao — nesse caso, truncar antes de redigir |
...
- **Fonte do handoff do DBA:** **nao houve handoff, e nao havera nesta versao.** Nao aplicavel por ausencia de mecanismo de persistencia, e nao por omissao — ver a invariante arquitetural declarada no inicio deste documento.
- **Premissas de crescimento:** o escopo aprovado **nao possui banco de dados nem qualquer estado local persistente**. A persistencia da aplicacao se limita a: arquivo de configuracao por perfil (ordem de grandeza de centenas de bytes, sem crescimento), cache de access token **em memoria** durante a execucao, acumulador de relatorio **em memoria** durante a execucao, e area temporaria transitoria dimensionada pelo tamanho de parte, removida ao final.
...
- **Acoes recomendadas:**
  1. **Manter esta secao como nao aplicavel nesta versao.** `DP-02` foi resolvida excluindo do MVP todas as funcionalidades que exigiriam persistencia, e `DP-09` esta fechada em consequencia. Nao ha pendencia a acompanhar aqui.
  2. **Criterio de aceite arquitetural associado:** a implementacao nao deve produzir escrita persistente fora do arquivo de configuracao. Verificavel por inspecao dos caminhos de escrita e por teste que confirma a ausencia de residuo apos execucao completa e apos interrupcao por sinal.
  3. **Gatilho de reabertura, para uso futuro.** Promover F-03 (retomada), F-06 (espelhamento), F-12 (trava de concorrencia) ou F-14 (deteccao por cursor) do backlog reabre `DP-09` e obriga a revisao desta secao. O impacto e proporcional a natureza do estado: um arquivo de trava ou cursor unico tem impacto desprezivel; um **catalogo de arquivos com resumos** cresce com a quantidade de itens monitorados e passa a exigir politica de crescimento, compactacao e recuperacao de consistencia — nesse caso, acionar o DBA para o handoff formal previsto no item 24 do protocolo comum, usando `.claude/agents-protocol/templates/plano-dimensionamento-expansao-banco-template.md`, ainda que o mecanismo seja armazenamento em arquivo e nao banco relacional.
...
1. **Publicar o arquivo de licenca (DP-20)** na raiz do repositorio antes do primeiro commit de codigo. E a unica pendencia que precede literalmente a primeira linha versionada.
2. **Iniciar pela camada de dominio**, integralmente destravada: `lib/hash` (contrato confirmado, com vetor de teste oficial), `lib/errors` e `lib/path`. Concentra o maior risco tecnico do projeto e nao depende de nenhuma decisao pendente. Sequencia completa em [decisoes-pendentes.md](../requisitos/decisoes-pendentes.md), secao "O que o Senior Developer precisa para comecar".
3. Fechar **DP-07** e **DP-08** antes de `lib/preflight`, `lib/json` e da camada de compatibilidade de utilitarios. Considerar em DP-08 tambem a dependencia de utilitario de resumo SHA-256 exigida por `lib/hash`.
4. Fechar **DP-11** e **DP-05** antes de `lib/config`.
