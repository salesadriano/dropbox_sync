...
| **RSK-23** | 🔄 **REESCRITO na v0.8 — o estado passou a ser autorizado.** ~~Reintroducao inadvertida de estado local persistente~~ **Estado persistente alem do delimitado.** `DP-06` autorizou estado para o `sync`, revogando `PRJ-DEC-07` nesse escopo. A vigilancia deixa de ser "nao criar estado" e passa a ser "nao exceder o estado autorizado" | Medio | Media | `RNF-26` delimita o autorizado: **linha de base de sincronizacao** e **cursor de enumeracao**, ambos vinculados ao par de raizes. Qualquer outra escrita persistente — cache de resumos, indice auxiliar, arquivo de trava — permanece fora de escopo e constitui mudanca de escopo. Verificavel por auditoria dos caminhos de escrita. **O invariante original continua valendo integralmente para `upload`, `download`, `list`, `delete` e `info`** | Tech Lead |
