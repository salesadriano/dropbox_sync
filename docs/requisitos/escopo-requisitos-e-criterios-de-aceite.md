...
| RF-53 | 🔴 **Contrato dos dois lados: papel por sinalizador, tipo por resolucao, ambiguidade recusada** (`DP-27`, `DP-27a`) | **P0** | **(a)** `--origem` e `--destino` sao obrigatorios e nomeiam o PAPEL; a posicao na linha de comando nao significa nada. **(b)** Exatamente **um** lado e local e **um** e remoto: par do mesmo tipo e recusado com diagnostico que diz qual regra foi violada. **(c)** O tipo e apurado por resolucao — e local o lado que resolve para **diretorio existente no sistema de arquivos**, e o outro e remoto por eliminacao. **(d)** Quando os **dois** resolvem para diretorio local, ou quando **nenhum** resolve, o comando **RECUSA** nomeando a ambiguidade, e nao escolhe. **(e)** O sentido decorre: origem local produz **envio**, origem remota produz **recebimento**, e o relatorio declara o sentido apurado antes de qualquer escrita. **Razao da recusa em (d):** classificar errado inverte o sentido do espelhamento e apaga a arvore oposta a pretendida. Adivinhar aqui e a unica decisao do `sync` cujo erro nao tem diagnostico posterior — quando aparece, ja aconteceu. **`DP-28` esta aberta** para a saida do operador diante dessa recusa |
...
| RF-37, RF-53 | `cmd/sync`, `lib/path` | TC-LADO-01..05 — `--origem`/`--destino` obrigatorios; par do mesmo tipo recusado; ambiguidade de tipo recusada nos dois sentidos; sentido apurado declarado antes de qualquer escrita |
