...
| Versao | **v0.3 — escopo do MVP fechado** |
| Status | ✅ **Escopo aprovado. Nenhuma decisao bloqueante em aberto.** Pendencias remanescentes sao estruturantes (P1), nao bloqueantes |
...
| RES-08 | Shell nao oferece parser JSON nativo; o tratamento de resposta precisa de estrategia explicita | Limitacao da linguagem imposta em RES-01 |
...
| RF-05 | Suporte a multiplos perfis de credencial na mesma instalacao, selecionaveis por parametro e por variavel de ambiente | P1 | **Condicional (DP-05)** | Dado dois perfis configurados, quando o mesmo comando e executado com cada perfil, entao cada execucao opera na conta correspondente e nenhuma le a configuracao da outra |
...
| ID | Requisito | Prior. | Status | Criterio de aceite |
|---|---|---|---|---|
| RF-16 | Listagem de pasta remota com paginacao completa via cursor | P0 | Derivavel | Dada pasta com mais itens do que uma pagina retorna, quando a listagem e executada, entao todos os itens sao apresentados por meio de `list_folder` e `list_folder/continue`, sem truncamento silencioso |
...
| RF-22 | Busca de itens por termo, com paginacao por cursor e deduplicacao de resultados | P1 | Derivavel | Dado termo com correspondencias conhecidas, quando a busca e executada, entao os resultados sao obtidos via `/2/files/search_v2` e paginados por `/2/files/search/continue_v2`, nenhuma chamada e feita ao endpoint sem sufixo, itens repetidos entre paginas sao apresentados uma unica vez (RES-14) e o teto de 10.000 correspondencias e comunicado ao usuario quando atingido |
...
| RF-28 | Saida com duas apresentacoes sobre o mesmo modelo de resultado: legivel por humano quando houver terminal associado, e estruturada legivel por maquina quando nao houver ou quando solicitada por sinalizador explicito | **P0** *(promovido por DP-03)* | Derivavel | Dado comando de consulta executado com terminal associado, quando concluido, entao a saida e formatada para leitura humana; dado o mesmo comando executado sem terminal associado ou com o sinalizador de saida estruturada, entao a saida padrao contem apenas registros parseaveis por script, sem texto decorativo, e todo diagnostico e escrito na saida de erro. O sinalizador explicito prevalece sobre a deteccao automatica em ambos os sentidos |
...
|---|---|---|---|
| RNF-01 | ⚠️ **EM CONFLITO — congelado ate DP-07 fechar.** A aplicacao deve executar em `bash` sem recursos exclusivos de versoes recentes, com versao minima declarada e verificada em tempo de execucao | Portabilidade | **Redacao original mantida deliberadamente.** O criterio da v0.1 exigia execucao em `bash` 3.2, e a implementacao da camada de dominio usa `declare -A`, `declare -g` e `${var,,}`, incompativeis com esse piso. **Este requisito nao sera reescrito antes de DP-07 ser respondida pelo solicitante** — ver a analise em DIV-15 e o piso real apurado |
| RNF-02 | Dependencias externas limitadas a `cURL` e utilitarios POSIX presentes em instalacao base; qualquer dependencia adicional deve ser declarada e verificada na inicializacao | Portabilidade | Dado ambiente com apenas shell, `cURL` e coreutils, quando os comandos essenciais sao executados, entao todos concluem com sucesso; dado utilitario obrigatorio ausente, entao a aplicacao falha com mensagem nomeando o utilitario |
...
| RNF-04 | Credenciais persistidas com permissao restrita ao proprietario e `umask` restritivo durante a execucao | Seguranca | Dado arquivo de configuracao criado pela aplicacao, quando as permissoes sao verificadas, entao o modo e `0600`; dado arquivo com permissao mais ampla, entao a aplicacao alerta ou recusa a leitura |
...
| RNF-11 | O tratamento de resposta JSON deve ser resistente a variacao de formatacao, ordem de campos e valores contendo delimitadores | Corretude | Dado corpo JSON com espacamento alterado, ordem de campos diferente e valor de texto contendo `"` e `:`, quando a resposta e interpretada, entao os campos extraidos permanecem corretos. **Divergencia conhecida: extracao por expressao regular como no projeto de referencia nao satisfaz este criterio** |
...
| RNF-22 | `lib/output`, `lib/log` | TC-RED-01..02 — identificador de correlacao em linha propria sobrevive a redacao; reprovacao se concatenado a cabecalho sensivel |
...
| **DIV-15** | ⚠️ **Divergencia de processo: decisao do solicitante antecipada pela implementacao.** A camada de dominio usa `declare -A`, `declare -g` e `${var,,}`, incompativeis com o piso de `bash` 3.2 declarado em RNF-01. A justificativa registrada afirmava que "o solicitante fixou Linux com `bash` 4+" — **isso nao ocorreu.** `DP-07 · Plataformas e shells suportados` continua **em aberto**. O que existe e "stack detectada: `bash` 4+" na memoria de projeto, que e **observacao do ambiente, nao decisao de escopo** | RNF-01 versus implementacao; painel de DP-07 | Uma decisao que cabe ao solicitante foi tomada de fato pela implementacao. O risco nao e o piso escolhido — provavelmente razoavel — mas o precedente: premissa de escopo criada por conveniencia tecnica e registrada como se fosse decisao | **1. Nao apagar.** A divergencia fica registrada como ocorrencia de processo. **2. `RNF-01` congelado**, com a redacao original preservada, ate DP-07 ser respondida. **3. Piso real apurado: `bash` 4.4**, e nao 4.0 como se supunha — `declare -g` exige **4.2**, e a harness de teste exige **4.4** por expandir `"${casos[@]}"` com array vazio sob `set -u`. Consequencia concreta: **RHEL 6 (`bash` 4.1) fica fora**, o que e informacao material para o solicitante decidir. **4.** O Business Analyst **nao** fecha DP-07 |
| **DIV-16** | 🔄 **Reclassificada na v0.4.** O conflito entre RNF-10 (nomes com quebra de linha) e RF-28/RF-35 (saida orientada a linha) estava registrado apenas como "conflito de formato a decidir antes de `lib/output`` | Ciclo de QA da camada de dominio | A classificacao anterior era incompleta e adiava indevidamente o tratamento | Eram **duas coisas distintas sob um rotulo so**. **(a) Defeito de implementacao ativo**, nao mera divergencia futura: `lib/path` retornava valor errado **com status 0** para caminho contendo nova linha — falha silenciosa, a pior classe. **Corrigido no ciclo (C2-01/D1) e verificado.** **(b) Divergencia de requisito, que segue aberta:** o formato de saida orientado a linha de RF-28 e RF-35 nao consegue representar sem ambiguidade um nome que contenha nova linha. Precisa de decisao de desenho — delimitador alternativo, escape ou terminador nulo — **antes de `lib/output`**, na Etapa 2. RNF-10 foi emendado para exigir explicitamente o caractere de nova linha no conjunto de teste |
