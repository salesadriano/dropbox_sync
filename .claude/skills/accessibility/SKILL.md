---
name: accessibility
description: Remediate accessibility issues in existing web interfaces following WCAG 2.2 guidelines. Use when fixing implemented UI, content, keyboard flows, or screen-reader behavior on the web. Prefer accessibility-review for review-only audits and accessibility-compliance for implementing broader accessible component patterns or mobile accessibility.
license: MIT
metadata:
...
- Use `accessibility-review` when the primary deliverable is an audit report or handoff assessment.
- Use `accessibility-compliance` when the primary task is implementing accessible component patterns, ARIA-heavy interactions, focus systems, or mobile accessibility support.
- Use this skill when the main need is to remediate accessibility issues in existing web UI and content after the problems are already known.
...
**No keyboard traps.** Users must be able to Tab into and out of every component. See `references/A11Y-PATTERNS.md` in the "Modal focus trap" section for a dialog pattern; the native `<dialog>` element handles this automatically.
...
Provide a skip link so keyboard users can bypass repetitive navigation. See `references/A11Y-PATTERNS.md` in the "Skip link" section for full markup and styles.
...
Any action that requires dragging must have a single-pointer alternative (e.g., buttons, inputs). See `references/A11Y-PATTERNS.md` in the "Dragging movements" section for a sortable-list example.
...
Every input needs a programmatically associated label. See `references/A11Y-PATTERNS.md` in the "Form labels" section for explicit, implicit, and instructional examples.

### Error handling (3.3.1, 3.3.3)

Announce errors to screen readers with `role="alert"` or `aria-live`, set `aria-invalid="true"` on invalid fields, and focus the first error on submit. See `references/A11Y-PATTERNS.md` in the "Error handling" section for full markup and JS.
...
**When ARIA is needed,** use the correct roles and states. See `references/A11Y-PATTERNS.md` in the "ARIA tabs" section for a complete tablist example.

### Live regions (4.1.3)

Use `aria-live` regions to announce dynamic content changes without moving focus. See `references/A11Y-PATTERNS.md` in the "Live regions and notifications" section for markup and a `showNotification()` helper.
...
See `references/A11Y-PATTERNS.md` in the "Screen reader commands" section for VoiceOver and NVDA shortcuts.
